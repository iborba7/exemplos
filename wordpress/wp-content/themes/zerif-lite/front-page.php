...
if ( get_option( 'show_on_front' ) == 'page' ) {
	...
	if(isset($_POST['submitted'])) :
			...
			if(trim($_POST['myname']) === ''):
				...
				$nameError = __('* Por favor insira seu nome.','zerif-lite');
			...
			if(trim($_POST['myemail']) === ''):
				...
				$emailError = __('* Por favor insira seu endereço de e-mail.','zerif-lite');
			...
			elseif (!preg_match("/^[[:alnum:]][a-z0-9_.-]*@[a-z0-9.-]+\.[a-z]{2,4}$/i", trim($_POST['myemail']))) :
				...
				$emailError = __('* Seu endereço de e-mail é inválido.','zerif-lite');
			...
			if(trim($_POST['mysubject']) === ''):
				...
				$subjectError = __('* Por favor insira um assunto.','zerif-lite');
			...
			if(trim($_POST['mymessage']) === ''):
				...
				$messageError = __('* Por favor insira uma mensagem.','zerif-lite');
				...
				<div class="row">
					...
					<form role="form" method="POST" action="" onSubmit="this.scrollPosition.value=(document.body.scrollTop || document.documentElement.scrollTop)" class="contact-form">
						...
						<div class="col-lg-4 col-sm-4 zerif-rtl-contact-name" data-scrollreveal="enter left after 0s over 1s">
							...
							<input required="required" type="text" name="myname" placeholder="<?php _e('Seu nome','zerif-lite'); ?>" class="form-control input-box" value="<?php if(isset($_POST['myname'])) echo esc_attr($_POST['myname']);?>">

						</div>

						<div class="col-lg-4 col-sm-4 zerif-rtl-contact-email" data-scrollreveal="enter left after 0s over 1s">

							<input required="required" type="email" name="myemail" placeholder="<?php _e('Seu e-mail','zerif-lite'); ?>" class="form-control input-box" value="<?php if(isset($_POST['myemail'])) echo is_email($_POST['myemail']) ? $_POST['myemail'] : ""; ?>">

						</div>

						<div class="col-lg-4 col-sm-4 zerif-rtl-contact-subject" data-scrollreveal="enter left after 0s over 1s">

							<input required="required" type="text" name="mysubject" placeholder="<?php _e('Assunto','zerif-lite'); ?>" class="form-control input-box" value="<?php if(isset($_POST['mysubject'])) echo esc_attr($_POST['mysubject']);?>">

						</div>

						<div class="col-lg-12 col-sm-12" data-scrollreveal="enter right after 0s over 1s">

							<textarea name="mymessage" class="form-control textarea-box" placeholder="<?php _e('Sua mensagem','zerif-lite'); ?>"><?php if(isset($_POST['mymessage'])) { echo esc_html($_POST['mymessage']); } ?></textarea>

						</div>
	
						<?php
							$zerif_contactus_button_label = get_theme_mod('zerif_contactus_button_label',__('Enviar mensagem','zerif-lite'));
